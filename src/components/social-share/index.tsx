...
export async function shareToSocial(
  ...
  const shareUrls: Record<string, string> = {
    ...
    tiktok: isMobile
      ? `tiktok://share?url=${encodedUrl}&text=${encodedText}`
      : `https://www.tiktok.com/@/share?url=${encodedUrl}&text=${encodedText}`,
  };
...
export function generateQRCodeUrl(
  ...
  const { size = 300, format = "png", margin = 1 } = options;
...
export async function shareWithAnalytics(
  ...
  if (analyticsCallback && result.success) {
    analyticsCallback({
      event: eventName,
      platform:
        platform || (isWebShareSupported() ? "web_share_api" : "clipboard"),
